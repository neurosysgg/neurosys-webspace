...
function sizeConsentGates() {
    for (const gate of queryAll('.player-consent[data-player-height]')) {
...
function wireCoverFallback() {
    for (const img of queryAll('.cover-art img[data-fallback]')) {
        if (img.dataset.fallbackWired)
            continue;
...
function wireConsentButtons() {
    for (const btn of queryAll('.player-consent-btn')) {
