 ...
 * {@link \NeuroSYS\Http\Security\CspScheme} makes and for the same kind of reason: the colon is
 ...
 * markup — the enums it would have sat beside there ({@link \NeuroSYS\View\Html\LinkRel},
 * {@link \NeuroSYS\View\Html\MetaName}, {@link \NeuroSYS\View\Html\ScriptType}) are all vocabularies
 * of HTML itself. Same placement argument {@link Charset} carries, and the same two-reader shape:
 * {@link \NeuroSYS\View\Html\Element} asks which schemes are allowed, and two views build a
 * `mailto:` address.
 *
 ...
 * {@link \NeuroSYS\Http\Security\StrictTransportSecurity} means this site does not emit one. No
 * `data:`, because a `data:text/html` document runs script in the origin that navigated to it. Both
 * absences are load-bearing — see {@link \NeuroSYS\View\Html\Element::URL_SCHEMES}, which is the
...
enum UrlScheme: string
{
     ...
     * happens on the way out anyway, in {@link \NeuroSYS\View\Html\Element::render()}, which is
