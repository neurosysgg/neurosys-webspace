...
- Fragments with behaviour are custom elements, server-rendered and enhanced in place, so they upgrade themselves when `Navigation` swaps `#content` — nothing re-initialises after a navigation.
