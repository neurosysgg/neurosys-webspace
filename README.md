...
| | Why |
|---|---|
| **PHP ≥ 8.5** with `ext-uri`, `ext-dom`, `ext-openssl`, `ext-zlib` | The pipe operator in `autoload.php`, `#[\NoDiscard]`, the URL parsers `Element` and `Request` use, the HTML parser `MarkupParser` reads the privacy policy with, and the signature and gzip that the signed `/admin` push is made of. All five are declared in `composer.json` and asked for by name in the verify script. |
