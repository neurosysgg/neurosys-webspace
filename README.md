...
| | Why |
|---|---|
| **PHP ≥ 8.5** with `ext-uri`, `ext-dom`, `ext-intl`, `ext-openssl`, `ext-zlib`, `ext-mbstring` | The pipe operator in `autoload.php`, `#[\NoDiscard]`, the URL parsers `Element` and `Request` use, the HTML parser `MarkupParser` reads the privacy policy with, the formatter every translated string goes through, the signature and gzip that the signed `/admin` push is made of, and the UTF-8 check every form read makes. All six are declared in `composer.json` and asked for by name in the verify script. |
